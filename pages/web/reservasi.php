...
{
	echo "<h1>Tanggal keluar tidak boleh kurang/sama dari tanggal masuk dan tipe villa harus diisi.</h1>
	<h2>Silakan gunakan widget sebelah kanan untuk mencoba kembali =>=>=></h2>";
...
if(isset($_POST['button222']))
{
	...
	$q=mysql_query("INSERT INTO `vcorners`.`reservations` (`resv_date`,`resv_date2`,`title`, `name`, `identify_kinds`, `identify_value`, `payment_kinds`, `payment_value`, `idclass`) VALUES ( '".$_SESSION['tgl1']."', '".$_SESSION['tgl2']."', '".$_POST['title']."', '".$_POST['nama']."', '".$_POST['identitas']."', '".$_POST['iden']."', '".$_POST['payment']."', '".$_POST['pay']."', '".$_SESSION['rm']."')");
	if($q)
	{
		...
		unset($_SESSION['rm']);
		unset($_SESSION['ada']);
		_direct("?y=".session_id()."&page="._encodeParam("about"));
	}
}
